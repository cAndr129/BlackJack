...
{
    // --- This script is for player AND dealer
    ...
    public int GetCard()
    {
        // Get a card, use deal card to assign sprites and value to card on table
        ...
        if (cardValue == 1)
        {
            aceList.Add(hand[cardIndex-1].GetComponent<CardScript>());
        }
    ...
    // Search for needed ace conversions
    public void AceCheck()
    {
        // for each ace in aceList
        foreach (CardScript ace in aceList)
        {
            if (handValue + 10 < 22 && ace.GetValueOfCard() == 1)
            {
                // if converting, adjust card object and hand
                ace.SetValue(11);
                handValue += 10;
    ...
    // Hides all cards, resets necessary vars
